...
import com.water.ui.WaterUILog;
 ...
 * @date 2016-03-17
 */
public class WaterUIDrawableHelper {
    private static final String TAG = WaterUIDrawableHelper.class.getSimpleName();
    ...
    public static Bitmap createBitmapFromView(View view, int leftCrop, int topCrop, int rightCrop, int bottomCrop) {
        Bitmap originBitmap = WaterUIDrawableHelper.createBitmapFromView(view);
    ...
    Drawable getVectorDrawable(Context context, @DrawableRes int resVector) {
        try {
            ...
            WaterUILog.d(TAG, "Error in getVectorDrawable. resVector=" + resVector + ", resName=" + context.getResources().getResourceName(resVector) + e.getMessage());
            return null;
        }
    }
