 ...
 * @date 2016-03-17
 */
public class WaterUIDisplayHelper {
    ...
    private static final String TAG = "WaterUIDisplayHelper";
    ...
    public static int getScreenHeight(Context context) {
        ...
        if(WaterUIDeviceHelper.isXiaomi() && xiaomiNavigationGestureEnabled(context)){
    ...
    public static int[] getRealScreenSize(Context context) {
        if (WaterUIDeviceHelper.isEssentialPhone() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
    ...
    public static int getUsefulScreenWidth(Activity activity) {
        return getUsefulScreenWidth(activity, WaterUINotchHelper.hasNotch(activity));
    }

    public static int getUsefulScreenWidth(View view) {
        return getUsefulScreenWidth(view.getContext(), WaterUINotchHelper.hasNotch(view));
    }
    ...
    public static int getUsefulScreenWidth(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isLandscape && WaterUIDeviceHelper.isEssentialPhone()
                ...
                result -= 2 * WaterUIStatusBarHelper.getStatusbarHeight(context);
            }
            return result;
        }
        if (isLandscape) {
            // 华为挖孔屏横屏时，会把整个 window 往后移动，因此，可用区域减小
            if (WaterUIDeviceHelper.isHuawei() && !WaterUIDisplayHelper.huaweiIsNotchSetToShowInSetting(context)) {
                result -= WaterUINotchHelper.getNotchSizeInHuawei(context)[1];
            }
    ...
    public static int getUsefulScreenHeight(Activity activity) {
        return getUsefulScreenHeight(activity, WaterUINotchHelper.hasNotch(activity));
    }

    public static int getUsefulScreenHeight(View view) {
        return getUsefulScreenHeight(view.getContext(), WaterUINotchHelper.hasNotch(view));
    }
    ...
    private static int getUsefulScreenHeight(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isPortrait && WaterUIDeviceHelper.isEssentialPhone()
                    && Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                // https://arstechnica.com/gadgets/2017/09/essential-phone-review-impressive-for-a-new-company-but-not-competitive/
                // 这里说挖孔屏是状态栏高度的两倍
                result -= 2 * WaterUIStatusBarHelper.getStatusbarHeight(context);
...
//        }
    ...
    public static int getStatusBarHeight(Context context) {
        if(WaterUIDeviceHelper.isXiaomi()){
